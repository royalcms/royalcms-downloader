...
namespace Royalcms\Component\Downloader;
...
class Downloader
{
            ...
            do {
                ...
                if ($curlInfo !== false) {
                    if ($curlInfo['result'] == \CURLE_OK) {
                        ...
                        throw new Exception("Transfer error: " . curl_error($curlInfo['handle']));
                    }
                }
            } while ($curlMessages);
     ...
     */
    private function _httpFileSize($url, $maxRedirs = 20, $cookie = null)
    {
        $ch = curl_init($url);

        curl_setopt_array($ch, array(\CURLOPT_NOBODY => true, \CURLOPT_RETURNTRANSFER => true, \CURLOPT_HEADER => true, \CURLOPT_FOLLOWLOCATION => true, \CURLOPT_MAXREDIRS => $maxRedirs, \CURLOPT_COOKIE => $cookie, \CURLOPT_CONNECTTIMEOUT => $this->networkTimeout, \CURLOPT_LOW_SPEED_TIME => $this->networkTimeout, \CURLOPT_LOW_SPEED_LIMIT => 1, \CURLOPT_USERAGENT => $this->userAgent));
    ...
    private function _allocateChunk()
    {
        ...
        $curChunkLength = min($this->chunkSize, $this->totalBytes - $this->chunkSize * $this->chunkIndex);
        $range          = $curChunkOffset . '-' . ($curChunkOffset + $curChunkLength - 1);

        $ch = curl_init($this->url);

        curl_setopt_array($ch, array(\CURLOPT_WRITEFUNCTION => array($this, '_writeData'), \CURLOPT_HEADER => false, \CURLOPT_RANGE => $range, \CURLOPT_CONNECTTIMEOUT => $this->networkTimeout, \CURLOPT_LOW_SPEED_TIME => $this->networkTimeout, \CURLOPT_LOW_SPEED_LIMIT => 1, \CURLOPT_COOKIE => $this->cookie, \CURLOPT_FOLLOWLOCATION => true, \CURLOPT_VERBOSE => $this->debugMode, \CURLOPT_USERAGENT => $this->userAgent));
    ...
    private function _writeData($curlHandle, $data)
    {
        ...
        if (! is_null($this->progressCallback)) {
            ...
            if ((microtime(true) - $this->lastProgressCallbackTime) >= $this->minCallbackPeriod) {
                if (false === call_user_func($this->progressCallback, $this->doneBytes, $this->totalBytes)) {
                    $this->break = true;
    ...
    public function getTotalBytes()
    {
        if (is_null($this->totalBytes)) {
            ...
            if (false === ($this->totalBytes = $this->_httpFileSize($this->url, $this->maxRedirs, $this->cookie))) {
